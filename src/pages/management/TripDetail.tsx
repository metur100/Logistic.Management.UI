...
interface Trip {
  ...
  podNumber?: string; podReceived?: boolean; remarks?: string
...
const STEP_SHORT: Record<string,string> = {
  ...
  InTransit:'Transit', NearDestination:'Near', Unloading:'Unlod', DeliveryCompleted:'Done'
}
...
function InfoRow({ label, value }: { label: string; value?: string | number | null }) {
  ...
  return (
    <div style={{
      ...
      padding: '.5rem 0', borderBottom: '1px solid var(--border)', alignItems: 'start'
    }}>
      <span style={{ color:'var(--text-muted)', fontSize:'.72rem', fontWeight:700,
        textTransform:'uppercase', letterSpacing:'.04em', paddingTop:'.05rem' }}>
        {label}
      </span>
      <span style={{ fontWeight:600, fontSize:'.8rem', textAlign:'right',
...
export default function TripDetail() {
  ...
  if (!trip) return (
    <div style={{ background:'var(--surface)', border:'1px solid var(--border)',
      borderRadius:'var(--radius)', textAlign:'center', padding:'3rem 1rem', color:'var(--text-muted)' }}>
  ...
  return (
    <div style={{ width:'100%', maxWidth:'100%', overflowX:'hidden', boxSizing:'border-box' }}>
      <style>{`
        .td-section-stack { display:flex; flex-direction:column; gap:.85rem; margin-bottom:.85rem; }
        .td-two-col { display:grid; grid-template-columns:1fr 1fr; gap:.85rem; margin-bottom:.85rem; }
        ...
        .td-timeline-dot { width:9px; height:9px; border-radius:50%; flex-shrink:0; margin-top:.4rem; }
      ...
      <div style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'.75rem', flexWrap:'nowrap' }}>
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ fontSize:'.65rem', opacity:.85, fontWeight:700, textTransform:'uppercase', letterSpacing:'.07em', marginBottom:'.25rem' }}>
              {trip.status}
            </div>
            <div style={{ fontSize:'1rem', fontWeight:800, marginBottom:'.3rem', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {trip.tripNumber}
            </div>
            <div style={{ fontSize:'.78rem', opacity:.9, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {trip.originLocation} → {trip.destinationLocation}
            </div>
            <div style={{ display:'flex', gap:'.75rem', marginTop:'.35rem', flexWrap:'wrap' }}>
              {trip.distanceKm && (
                <span style={{ fontSize:'.72rem', opacity:.8 }}>📍 {trip.distanceKm} km</span>
              )}
              {(trip.cargoItems?.length ?? 0) > 0 && (
                <span style={{ fontSize:'.72rem', opacity:.8 }}>📦 {trip.cargoItems!.length} cargo</span>
              )}
              {totalWeight > 0 && (
                <span style={{ fontSize:'.72rem', opacity:.8 }}>⚖️ {totalWeight.toFixed(2)} t</span>
              )}
            </div>
          </div>
          <button onClick={() => nav(`/management/trips/${id}/edit`)} style={{
            ...
            padding:'.4rem .75rem', cursor:'pointer', fontSize:'.78rem',
      ...
      {trip.status !== 'Cancelled' && (
        <div style={{
          ...
          marginBottom:'.85rem', overflowX:'auto', WebkitOverflowScrolling:'touch' as any
        }}>
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', minWidth:360, gap:0 }}>
            {STEPS.map((step, idx) => {
              ...
              return (
                <div key={step} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', position:'relative' }}>
                  {idx < STEPS.length - 1 && (
                    <div style={{ position:'absolute', top:13, left:'50%', right:'-50%', height:2,
                      background: idx < currentStepIdx ? '#22c55e' : 'var(--border)', zIndex:0 }} />
                  )}
                  <div style={{
                  ...
                  <div style={{ fontSize:'.58rem', fontWeight:600, color:textColor, marginTop:'.3rem', textAlign:'center', lineHeight:1.2 }}>
                    {STEP_SHORT[step]}
                  </div>
                </div>
              )
      ...
      {/* ── Top 3-col: Trip Info | Driver & Vehicle | Loading Times ── */}
      <div className="td-three-col">
        ...
        <SectionCard title={t('trip_info')} icon="📋">
          ...
          {trip.podReceived && (
            <div style={{ marginTop:'.75rem', padding:'.6rem .8rem', background:'#f0fdf4',
              borderRadius:8, border:'1px solid #bbf7d0', display:'flex', alignItems:'center', gap:'.6rem' }}>
              <span>✅</span>
              <div>
                <div style={{ fontWeight:700, color:'#16a34a', fontSize:'.78rem' }}>{t('pod_received')}</div>
                {trip.podNumber && <div style={{ fontSize:'.72rem', color:'var(--text-muted)' }}>{trip.podNumber}</div>}
              </div>
            </div>
          )}
          {trip.remarks && (
            <div style={{ marginTop:'.75rem', padding:'.6rem .8rem', background:'#fffbeb',
              borderRadius:8, border:'1px solid #fde68a', fontSize:'.78rem', color:'#92400e', wordBreak:'break-word' }}>
              📝 {trip.remarks}
            </div>
          )}
        </SectionCard>
        ...
        <SectionCard title={t('driver_vehicle')} icon="👤">
          {trip.driver?.fullName ? (
            <>
              <div style={{ display:'flex', alignItems:'center', gap:'.6rem', padding:'.6rem',
                background:'var(--surface2)', borderRadius:8, marginBottom:'.6rem' }}>
                ...
                <div style={{ minWidth:0 }}>
                  ...
                  {trip.driver.phone && (
                    <div style={{ fontSize:'.72rem', color:'var(--text-muted)' }}>📞 {trip.driver.phone}</div>
                  )}
                </div>
              </div>
            ...
            <p style={{ color:'var(--text-muted)', fontSize:'.82rem', textAlign:'center', padding:'.5rem' }}>—</p>
          )}

          {trip.vehicle?.registrationNumber ? (
            <div style={{ marginTop:'.6rem', padding:'.6rem', background:'var(--surface2)',
              ...
              <div style={{ minWidth:0 }}>
                ...
                <div style={{ fontSize:'.72rem', color:'var(--text-muted)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  {[trip.vehicle.fleetName, trip.vehicle.type,
                    trip.vehicle.capacity ? `${trip.vehicle.capacity}t` : null].filter(Boolean).join(' · ')}
                </div>
              </div>
            </div>
          ) : (
            <p style={{ color:'var(--text-muted)', fontSize:'.82rem', textAlign:'center', padding:'.5rem', marginTop:'.5rem' }}>—</p>
          )}
        </SectionCard>
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
          gap:'.5rem', marginBottom:'.85rem', flexWrap:'wrap' }}>
        ...
        {(!trip.cargoItems || trip.cargoItems.length === 0) ? (
          <div style={{ textAlign:'center', padding:'1.5rem', color:'var(--text-muted)',
            ...
            <div style={{ fontWeight:600, fontSize:'.85rem' }}>{t('no_cargo')}</div>
          </div>
        ) : (
          <>
            {/* Desktop table */}
              ...
              <table style={{ minWidth:520, width:'100%' }}>
                ...
                <tbody>
                  {trip.cargoItems!.map((c, idx) => (
                    <tr key={c.id}>
                      <td style={{ fontWeight:700, color:'var(--text-muted)', fontSize:'.8rem' }}>{idx+1}</td>
                      ...
                      <td style={{ fontSize:'.82rem' }}>{c.consignee || '—'}</td>
                      <td style={{ maxWidth:140, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontSize:'.78rem', color:'var(--text-muted)' }}>
            ...
            <div className="cargo-cards-wrap">
              {trip.cargoItems!.map((c, idx) => (
                <div key={c.id} style={{
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:700, fontSize:'.82rem', marginBottom:'.25rem', wordBreak:'break-word' }}>
                      {c.description}
                    </div>
              ...
              {totalWeight > 0 && (
                <div style={{ textAlign:'right', padding:'.5rem .25rem',
                  fontWeight:800, color:'#16a34a', fontSize:'.85rem' }}>
                  ⚖️ {t('total')}: {totalWeight.toFixed(2)} t
                </div>
              )}
            </div>
            ...
            return (
              <div key={h.id} className="td-timeline-item">
                ...
                <div style={{ background:'var(--surface2)', borderRadius:8,
                  padding:'.6rem .8rem', border:'1px solid var(--border)', flex:1, minWidth:0 }}>
                  ...
                  {h.remarks && (
                    <div style={{ marginTop:'.3rem', fontSize:'.75rem', color:'var(--text)', wordBreak:'break-word' }}>
                      {h.remarks}
                    </div>
                  )}
                </div>
